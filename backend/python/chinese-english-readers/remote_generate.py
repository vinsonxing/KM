...
from scripts.read_notes import get_remote_notes, copy_file_to_remote_folder, copy_file_to_local
...
async def flow(user, host=HOSTNAME, times=1):
    ...
    files = get_remote_notes(host, USERNAME, PASSWORD, get_path(user), last_x_days=7000)
    if not files:
        return
    ...
    for file in files:
        copy_file_to_local(host, USERNAME, PASSWORD, get_path(user), file['name'], TMP_FOLDER)
        ...
        copy_file_to_remote_folder(host, USERNAME, PASSWORD, get_path(user), audio_file)
        # copy pdf at the last step, ensure the audio is generated successfully
        copy_file_to_remote_folder(host, USERNAME, PASSWORD, get_path(user), pdf_file)
